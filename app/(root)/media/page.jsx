...
const MediaPage = () => {
  ...
  useEffect(() => {
    const totalSizeMB = userfiledata
      .filter((item) =>
              filetype.media.some((fileType) => fileType.type === item.mimetype)
            )
    ...
    <div>
      ...
      <div className="grid grid-cols-4 justify-start gap-5 mt-5">
        {userfiledata.map((item) => {
          ...
          const filetypeimage = filetype.media.find((file) => file.type === item.mimetype);

          const fileimage = filetypeimage ? filetypeimage.image : '/path/to/default/image.png';

          if (filetype.media.some((file) => file.type === item.mimetype)) {
            return (

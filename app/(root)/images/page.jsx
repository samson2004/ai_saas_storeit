...
const ImagePage = () => {
  ...
  useEffect(() => {
    const totalSizeMB = userfiledata
      .filter((item) =>
              filetype.images.some((fileType) => fileType.type === item.mimetype)
            )
  ...
  return (
    <div>
      ...
      <div className="grid grid-flow-col-dense justify-start gap-5 mt-5">
        {userfiledata.map((item) => {
          ...
          if (filetype.images.some((file) => file.type === item.mimetype)) {
            const imagesrc = `/${item.filereferenceid}`;
            return (
              <div key={item._id} className="w-fit h-fit bg-white rounded-xl p-5">
                <div className="flex justify-between p-3 items-start">
                  <div className="p-4">
                    <div className="w-[60px] h-[60px] rounded-full overflow-hidden bg-black">
                      <Image
                        src={imagesrc}
                        width={60}
                        height={60}
